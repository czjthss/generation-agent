...
- `generation_agent/planner.py`: 将一句话描述转换为生成计划。计划会包含 `domain` 和 `generator_type`，用于选择领域生成机制。设置 `OPENAI_API_KEY` 后优先使用 LangChain Agent 调用特征工具；否则使用内置启发式规划器。
...
python -m generation_agent.cli "生成北京市夏季降水" \
  ...
  --length 720 \
  --output outputs/beijing_rain.csv
```
...
目录中包含 `series_*.csv`、合并长表 `dataset.csv`、`scenarios.json` 和包含计划及校验信息的 `manifest.json`。
...
python -m generation_agent.cli "生成同类工业负载" \
  --reference reference_load.csv \
  --length 1000 \
  --output outputs/synthetic_load.csv
```
...
- 数据集模式：填写领域、输出文件夹和序列数量，生成多个独立 `.arrow` 分片、场景定义与清单文件；不再重复生成合并 `dataset.csv`。
- 公共参数：序列长度、频率、起始时间、随机种子、模型、参考 CSV、异常开关和异常强度。
...
- `generate_time_series(..., anomalies="auto", anomaly_severity=None, ...)`: 直接返回 CSV，并允许覆盖异常开关和强度。
- `generate_time_series_code(...)`: 返回独立 Python 生成脚本。
- `synthesize_from_plan(plan_json, ...)`: 从计划 JSON 生成 CSV。
